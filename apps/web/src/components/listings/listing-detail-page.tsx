...
type Listing = { id: string; listingNo: string; title: string; description: string; price: number; currency: string; listingType: string; propertyType: string; status: string; city: string; district: string; neighborhood?: string | null; address: string; residentialDetails?: Record<string, unknown> | null; media: Array<{ id: string; url: string; isCover?: boolean; originalName?: string | null }>; features: Record<string, Feature[]> };
type Publication = { id: string; portalName: string; portalAccountId?: string | null; accountName?: string | null; status: string; lastError?: string | null; publishedAt?: string | null; lastAttemptAt?: string | null; updatedAt?: string | null; externalUrl?: string | null };
type PortalAccount = { id: string; connectionStatus: string; lastCheckedAt?: string | null; lastError?: string | null; portal: { name: string; code: string } };
type AuditLog = { id: string; action: string; createdAt: string; changes?: Record<string, unknown> | null; actor?: { firstName: string; lastName: string } | null };
type QueueResponse = { queuedJobCount?: number; publications?: Array<{ id: string }> };

const labels: Record<string, string> = { DRAFT: 'Taslak', ACTIVE: 'Aktif', ARCHIVED: 'Arşivlendi', PUBLISHING: 'Yayınlanıyor', QUEUED: 'Sırada', PROCESSING: 'Yayınlanıyor', PUBLISHED: 'Yayınlandı', FAILED: 'Başarısız', UPDATE_REQUIRED: 'Güncelleme gerekli', PENDING: 'Bekliyor', CONNECTED: 'Bağlandı' };
const featureGroups: Array<[string, string]> = [['facades', 'Cephe'], ['interiorFeatures', 'İç Özellikler'], ['exteriorFeatures', 'Dış Özellikler'], ['nearbyPlaces', 'Yakın Yerler'], ['transportation', 'Ulaşım'], ['views', 'Manzara'], ['accessibilityFeatures', 'Erişilebilirlik']];
...
const detailOrder = Object.keys(detailLabels);
const valueLabels: Record<string, string> = { AIR_CONDITIONING: 'Klima', OPEN: 'Açık Mutfak', OWNER_OCCUPIED: 'Ev Sahibi Oturuyor', CONDOMINIUM_EASEMENT: 'Kat İrtifaklı', AGENT: 'Emlak Danışmanı', STUDIO: 'Stüdyo', VALET: 'Vale Otopark' };
...
function humanizeValue(key: string, value: unknown, currency = 'TRY'): string {
  if (typeof value === 'boolean') return value ? (key.startsWith('has') ? 'Var' : 'Evet') : (key.startsWith('has') ? 'Yok' : 'Hayır');
  if (typeof value === 'string') return valueLabels[value] ?? value.replaceAll('_', ' ');
  if (typeof value === 'number' && ['grossArea', 'netArea'].includes(key)) return `${value} m²`;
  if (typeof value === 'number' && key === 'monthlyFee') return money(value, currency);
  return String(value);
}

function detailItems(details: Record<string, unknown> | null | undefined, currency: string) {
  if (!details) return [];
  const known = detailOrder.filter((key) => Object.prototype.hasOwnProperty.call(details, key));
  const unknown = Object.keys(details).filter((key) => key !== 'listingId' && !detailLabels[key]);
  return [...known, ...unknown]
    .map((key) => ({ key, label: detailLabels[key] ?? key, value: details[key] }))
    .filter((item) => item.value !== null && item.value !== undefined && item.value !== '')
    .map((item) => ({ ...item, value: humanizeValue(item.key, item.value, currency) }));
}
...
export function ListingDetailPage() {
  ...
  const republishablePublications = useMemo(() => publications.filter((item) => republishableStatuses.has(item.status)), [publications]);
  ...
  async function changeStatus(status: 'ARCHIVED' | 'DRAFT') {
    ...
    setError(null);
    try {
      const response = await authenticatedFetch(`listings/${listing.id}/status`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) });
  ...
  async function publish() {
    if (!listing || action) return;
    if (!hasMedia) { setError('İlanı yayınlamadan önce en az bir görsel yüklemelisiniz.'); return; }
    if (!selectedPortalIds.length) { setError('Yayınlamak için en az bir bağlı portal hesabı seçin.'); return; }
    setAction('publish');
    setError(null);
    setNotice('Yayın kuyruğuna alınıyor...');
    try {
      const response = await authenticatedFetch(`listings/${listing.id}/publish`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ portalAccountIds: selectedPortalIds }) });
      if (!response.ok) throw new Error(await safeMessage(response));
      ...
      setNotice(null);
      setError(e instanceof Error ? e.message : 'Yayınlama işlemi tamamlanamadı. Lütfen tekrar deneyin.');
    } finally {
      setAction(null);
    }
  }
  ...
  async function republish() {
    if (!listing || action) return;
    if (!hasMedia) { setError('İlanı yeniden yayınlamadan önce en az bir görsel yüklemelisiniz.'); return; }
    if (!selectedPublicationIds.length) { setError('Yeniden göndermek için en az bir yayın kaydı seçin.'); return; }
    setAction('republish');
    setError(null);
    setNotice('Yeniden yayına gönderiliyor...');
    try {
      const response = await authenticatedFetch(`listings/${listing.id}/republish`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ publicationIds: selectedPublicationIds }) });
      if (!response.ok) throw new Error(await safeMessage(response));
      const data = await response.json() as QueueResponse;
      ...
      setNotice(null);
      setError(e instanceof Error ? e.message : 'Yeniden yayınlama işlemi tamamlanamadı. Lütfen tekrar deneyin.');
    } finally {
      setAction(null);
    }
  }
  ...
  const housingDetails = detailItems(listing.residentialDetails, listing.currency);
