...
export function AuthProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => { let mounted = true; (async () => { const [accessToken, refreshToken] = await Promise.all([SecureStore.getItemAsync(ACCESS_KEY), SecureStore.getItemAsync(REFRESH_KEY)]); if (!mounted) return; if (accessToken && refreshToken) { setTokens({ accessToken, refreshToken }); try { const response = await apiRequest('users/me', accessToken); if (response.ok) setUser(await response.json()); else if (response.status === 401) { const fresh = await refreshTokens(refreshToken); await persist(fresh); const me = await apiRequest('users/me', fresh.accessToken); if (!me.ok) throw new Error('Oturum doğrulanamadı.'); setUser(await me.json()); } else throw new Error('Oturum doğrulanamadı.'); } catch { await clear(); } } setReady(true); })(); return () => { mounted = false; }; }, [clear, persist]);

  const signIn = useCallback(async (email: string, password: string) => { const result = await authLogin(email, password); await persist(result.tokens); const me = await apiRequest('users/me', result.tokens.accessToken); if (!me.ok) { await clear(); throw new Error('Kullanıcı profili alınamadı.'); } setUser(await me.json()); }, [clear, persist]);
